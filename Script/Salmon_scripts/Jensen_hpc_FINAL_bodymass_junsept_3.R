...
#Loop foreach 
OUOUmodel.uppertridiagENVjs <- foreach(i=1:100) %dopar% { #calculation for each iteration 
  ...
  result$result1 <- seed
  result$result2 <- fit.multivariate.OU(multilnmorpholnenvi,A.matrix ="upper.tri", R.matrix = "diag", trace=TRUE, iterations=1, iter.sd=0.000001)
...
for (j in 1:100) {
  ...
  AICvalues[j,2] = OUOUmodel.uppertridiagENVjs[[j]]$result2$AIC #AIC value
}
...
BESTRESULT=OUOUmodel.uppertridiagENVjs[[BestAICiteration]]
BESTRESULT




save.image(file='Jensen_FINAL_junsept_3.RData')
