...
#Loop foreach 
OUOUmodel.indepdiagENVIjs <- foreach(i=1:100) %dopar% { #calculation for each iteration 
  ...
  result$result1 <- seed
  result$result2 <- fit.multivariate.OU(multilnmorpholnenvi,A.matrix ="diag", R.matrix = "diag", trace=TRUE, iterations=1, iter.sd=0.000001) 
...
for (j in 1:100) {
  ...
  AICvalues[j,2] = OUOUmodel.indepdiagENVIjs[[j]]$result2$AIC #AIC value
}
...
BESTRESULT=OUOUmodel.indepdiagENVIjs[[BestAICiteration]]
BESTRESULT



save.image(file='Jensen_FINAL_junsept_2.RData')
