###################################################
##                                               ##
##  SALMON SCRIPT - Model 2a (iteration 1 - 50)  ##
##                                               ## 
###################################################
...
salmon_model_2a_it1 <- foreach(i=1:80) %dopar% { 
...
save.image(file = 'salmon_allruns_model_2a_it1.RData')



###### EXTRACTING AND SAVING THE ITERATION WITH BEST PARAMETER ESTIMATES ######
#Filtering 50 iterations which converged
salmon_model_2a_it1_filtered <- Filter(function(iteration) !is.na(iteration$result2[[1]]), salmon_model_2a_it1)
salmon_model_2a_it1_filtered <- Filter(function(iteration) !is.null(iteration), salmon_model_2a_it1_filtered)
salmon_model_2a_it1_results <- salmon_model_2a_it1_filtered[1:50]
...
for (j in 1:50) {
  AICc[j,1] <- j 
  AICc[j,2] <- salmon_model_2a_it1_results[[j]]$result2$AICc 
}
...
salmon_model_2a_best_it1 <- salmon_model_2a_it1_results[[AICc_min]]

#Saving the best iteration
print(salmon_model_2a_best_it1)
save(salmon_model_2a_best_it1, file = 'salmon_bestrun_model_2a_it1.RData')