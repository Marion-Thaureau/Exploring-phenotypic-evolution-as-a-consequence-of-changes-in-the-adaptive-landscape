#####################################################
##                                                 ##
##  SALMON SCRIPT - Model 2a (iteration 50 - 100)  ##
##                                                 ## 
#####################################################
...
#Model_2a: only salmon bodymass as an OU in A matrix; diagonal R matrix 
A <- matrix(c(1,0,0,0), nrow = 2, byrow=TRUE)
R <- matrix(c(1,0,0,1), nrow = 2, byrow=TRUE)   
 
#Loop for 65 iterations 
salmon_model_2a_it50 <- foreach(i=1:80) %dopar% { 
...
save.image(file = 'salmon_allruns_model_2a_it50.RData')



###### EXTRACTING AND SAVING THE ITERATION WITH BEST PARAMETER ESTIMATES ######
#Filtering 50 iterations which converged
salmon_model_2a_it50_filtered <- Filter(function(iteration) !is.na(iteration$result2[[1]]), salmon_model_2a_it50)
salmon_model_2a_it50_filtered <- Filter(function(iteration) !is.null(iteration), salmon_model_2a_it50_filtered)
salmon_model_2a_it50_results <- salmon_model_2a_it50_filtered[1:50]
...
for (j in 1:50) {
  AICc[j,1] <- j 
  AICc[j,2] <- salmon_model_2a_it50_results[[j]]$result2$AICc 
}
...
salmon_model_2a_best_it50 <- salmon_model_2a_it50_results[[AICc_min]]

#Saving the best iteration
print(salmon_model_2a_best_it50)
save(salmon_model_2a_best_it50, file = 'salmon_bestrun_model_2a_it50.RData')