#####################################################
##                                                 ##
##  SALMON SCRIPT - Model 3a (iteration 50 - 100)  ##
##                                                 ## 
#####################################################
...
salmon_model_3a_it50 <- foreach(i=1:80) %dopar% { 
...
#Model 3a: diagonal A matrix; diagonal R matrix
  result$result2 <- tryCatch({fit.multivariate.OU(ln.bodymass_ln.discharge_multiobj,A.matrix ="diag", R.matrix = "diag", iterations=1, iter.sd=0.000001, hess=TRUE) 
...
save.image(file = 'salmon_allruns_model_3a_it50.RData')



###### EXTRACTING AND SAVING THE ITERATION WITH BEST PARAMETER ESTIMATES ######
#Filtering 50 iterations which converged
salmon_model_3a_it50_filtered <- Filter(function(iteration) !is.na(iteration$result2[[1]]), salmon_model_3a_it50)
salmon_model_3a_it50_filtered <- Filter(function(iteration) !is.null(iteration), salmon_model_3a_it50_filtered)
salmon_model_3a_it50_results <- salmon_model_3a_it50_filtered[1:50]
...
for (j in 1:50) {
  AICc[j,1] <- j 
  AICc[j,2] <- salmon_model_3a_it50_results[[j]]$result2$AICc 
}
...
salmon_model_3a_best_it50 <- salmon_model_3a_it50_results[[AICc_min]]

#Saving the best iteration
print(salmon_model_3a_best_it50)
save(salmon_model_3a_best_it50, file = 'salmon_bestrun_model_3a_it50.RData')