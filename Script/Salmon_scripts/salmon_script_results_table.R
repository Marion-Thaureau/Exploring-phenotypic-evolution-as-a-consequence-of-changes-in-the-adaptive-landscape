...
salmon_model_1a_best$A <- matrix(0, nrow = 2, ncol = 2)
salmon_model_1a_best$optima <- rep(NA, 2)

salmon_model_1b_best <- salmon_model_1b
salmon_model_1b_best$A <- matrix(0, nrow = 2, ncol = 2)
salmon_model_1b_best$optima <- rep(NA, 2)
...
for (model in models_list) {
  ...
  anc_envi_list[model] <- get(model_name)$ancestral.values[[2]]
  opt_trait_list[model] <- get(model_name)$optima[[1]]
  opt_envi_list[model] <- get(model_name)$optima[[2]]
}
...
salmon_result_table <- data.frame(
...
write.csv(salmon_result_table, file = "../salmon_results_table.csv", row.names = FALSE)
