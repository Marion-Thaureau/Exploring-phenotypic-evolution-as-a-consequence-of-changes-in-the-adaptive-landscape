#####################################################
##                                                 ##
##  SALMON SCRIPT - Model 5b (iteration 50 - 100)  ##
##                                                 ## 
#####################################################
...
salmon_model_5b_it50 <- foreach(i=1:80) %dopar% { 
...
#Model_5b: upper tri in A matrix; full R matrix
  result$result2 <- tryCatch({fit.multivariate.OU(ln.bodymass_ln.discharge_multiobj, A.matrix = "upper.tri", R.matrix = "symmetric", iterations=1, iter.sd=0.000001, hess=TRUE) 
...
save.image(file = 'salmon_allruns_model_5b_it50.RData')



###### EXTRACTING AND SAVING THE ITERATION WITH BEST PARAMETER ESTIMATES ######
#Filtering 50 iterations which converged
salmon_model_5b_it50_filtered <- Filter(function(iteration) !is.na(iteration$result2[[1]]), salmon_model_5b_it50)
salmon_model_5b_it50_filtered <- Filter(function(iteration) !is.null(iteration), salmon_model_5b_it50_filtered)
salmon_model_5b_it50_results <- salmon_model_5b_it50_filtered[1:50]
...
for (j in 1:50) {
  AICc[j,1] <- j 
  AICc[j,2] <- salmon_model_5b_it50_results[[j]]$result2$AICc 
}
...
salmon_model_5b_best_it50 <- salmon_model_5b_it50_results[[AICc_min]]

#calculating the correlation coefficients 
Rmatrix_best_salmon_model_5b_it50 <- salmon_model_5b_best_it50$result2$R
cormatrix_best_salmon_model_5b_it50 <- cov2cor(Rmatrix_best_salmon_model_5b_it50)

#Saving the best iteration
print(salmon_model_5b_best_it50)
print(cormatrix_best_salmon_model_5b_it50)
save(salmon_model_5b_best_it50, cormatrix_best_salmon_model_5b_it50, file = 'salmon_bestrun_model_5b_it50.RData')