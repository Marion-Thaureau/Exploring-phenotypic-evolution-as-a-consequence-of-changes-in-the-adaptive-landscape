###########################################################
##                                                       ##
##  FORAMINIFERA SCRIPT - Model 2b (iteration 50 - 100)  ##
##                                                       ## 
###########################################################
...
foraminifera_model_2b_it50 <- foreach(i=1:104) %dopar% { 
...
save.image(file = 'foraminifera_allruns_model_2b_it50.RData')



###### EXTRACTING AND SAVING THE ITERATION WITH BEST PARAMETER ESTIMATES ######
#Filtering 50 iterations which converged
foraminifera_model_2b_it50_filtered <- Filter(function(iteration) !is.na(iteration$result2[[1]]), foraminifera_model_2b_it50)
foraminifera_model_2b_it50_filtered <- Filter(function(iteration) !is.null(iteration), foraminifera_model_2b_it50_filtered)
foraminifera_model_2b_it50_results <- foraminifera_model_2b_it50_filtered[1:50]
...
for (j in 1:50) {
  AICc[j,1] <- j 
  AICc[j,2] <- foraminifera_model_2b_it50_results[[j]]$result2$AICc 
}
...
foraminifera_model_2b_best_it50 <- foraminifera_model_2b_it50_results[[AICc_min]]

#calculating the correlation coefficients 
Rmatrix_best_foraminifera_model_2b_it50 <- foraminifera_model_2b_best_it50$result2$R
cormatrix_best_foraminifera_model_2b_it50 <- cov2cor(Rmatrix_best_foraminifera_model_2b_it50)

#Saving the best iteration
print(foraminifera_model_2b_best_it50)
print(cormatrix_best_foraminifera_model_2b_it50)
save(foraminifera_model_2b_best_it50, cormatrix_best_foraminifera_model_2b_it50, file = 'foraminifera_bestrun_model_2b_it50.RData')