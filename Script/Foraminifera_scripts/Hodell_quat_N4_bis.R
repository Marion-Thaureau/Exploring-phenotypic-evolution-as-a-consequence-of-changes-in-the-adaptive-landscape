...
#Loop foreach 
OUOU.model1 <- foreach(i=51:100) %dopar% { #calculation for each iteration 
  ...
  result$result2 <- fit.multivariate.OU.user.defined(multiENVI, A.user=A,R.user=R, iterations = 1, hess = TRUE, 
                                                     user.init.diag.A = c(29.67556,0.2847143,14.204215),
						     user.init.upper.diag.A = c(-25.6442576,7.481514))
...
save.image(file='Hodell_quat_N4_bis.RData')
...
AICvalues<-matrix(ncol=2,nrow=50)
