...
#Loop foreach 
OUOUmodel.uptrid13C <- foreach(i=1:100) %dopar% { #calculation for each iteration 
  ...
  result$result1 <- seed
  result$result2 <- fit.multivariate.OU(multid13Ci,A.matrix ="upper.tri", R.matrix = "diag", trace=TRUE, iterations=1)
...
for (j in 1:100) {
  ...
  AICvalues[j,2] = OUOUmodel.uptrid13C[[j]]$result2$AIC #AIC value
}
...
BESTRESULT=OUOUmodel.uptrid13C[[BestAICiteration]]
BESTRESULT



save.image(file='Hodell_Length_d13C_3.RData')
