...
#Loop foreach 
OUOUmodel.indepd18O <- foreach(i=1:100) %dopar% { #calculation for each iteration 
  ...
  result$result1 <- seed
  result$result2 <- fit.multivariate.OU(multid18Oi,A.matrix ="diag", R.matrix = "diag", trace=TRUE, iterations=1)
...
for (j in 1:100) {
  ...
  AICvalues[j,2] = OUOUmodel.indepd18O[[j]]$result2$AIC #AIC value
}
...
BESTRESULT=OUOUmodel.indepd18O[[BestAICiteration]]
BESTRESULT



save.image(file='Hodell_Length_d18O_2.RData')
