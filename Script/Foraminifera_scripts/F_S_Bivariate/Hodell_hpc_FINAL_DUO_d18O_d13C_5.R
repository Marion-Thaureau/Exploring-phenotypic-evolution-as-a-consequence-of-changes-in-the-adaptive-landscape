...
#Loop foreach 
OUOUmodel.lotrienvi <- foreach(i=1:100) %dopar% { #calculation for each iteration 
  ...
  result$result1 <- seed
  result$result2 <- fit.multivariate.OU(multiENVI,A.matrix ="lower.tri", R.matrix = "diag", trace=TRUE, iterations=1)
...
for (j in 1:100) {
  ...
  AICvalues[j,2] = OUOUmodel.lotrienvi[[j]]$result2$AIC #AIC value
}
...
BESTRESULT=OUOUmodel.lotrienvi[[BestAICiteration]]
BESTRESULT




save.image(file='Hodell_d18O_d13C_5.RData')
