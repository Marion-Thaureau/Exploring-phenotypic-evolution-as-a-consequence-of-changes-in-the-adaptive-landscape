...
#Loop foreach 
OUOUmodel.uptrienvi <- foreach(i=1:100) %dopar% { #calculation for each iteration 
  ...
  result$result1 <- seed
  result$result2 <- fit.multivariate.OU(multiENVI,A.matrix ="upper.tri", R.matrix = "diag", trace=TRUE, iterations=1) 
...
save.image(file='Hodell_d18O_d13C_3.RData')
...
for (j in 1:100) {
  ...
  AICvalues[j,2] = OUOUmodel.uptrienvi[[j]]$result2$AIC #AIC value
}
...
BESTRESULT=OUOUmodel.uptrienvi[[BestAICiteration]]
BESTRESULT
