...
#Loop foreach 
OUOUmodel.lorowenvi <- foreach(i=1:100) %dopar% { #calculation for each iteration 
...
save.image(file='Hodell_d18O_d13C_6.RData')
...
for (j in 1:100) {
  ...
  AICvalues[j,2] = OUOUmodel.lorowenvi[[j]]$result2$AIC #AIC value
}
...
BESTRESULT=OUOUmodel.lorowenvi[[BestAICiteration]]
BESTRESULT
