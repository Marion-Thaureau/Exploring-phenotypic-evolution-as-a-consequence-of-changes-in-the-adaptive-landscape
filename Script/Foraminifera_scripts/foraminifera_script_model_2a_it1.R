#########################################################
##                                                     ##
##  FORAMINIFERA SCRIPT - Model 2a (iteration 1 - 50)  ##
##                                                     ## 
#########################################################
...
foraminifera_model_2a_it1 <- foreach(i=1:260) %dopar% { 
...
save.image(file = 'foraminifera_allruns_model_2a_it1.RData')



###### EXTRACTING AND SAVING THE ITERATION WITH BEST PARAMETER ESTIMATES ######
#Filtering 50 iterations which converged
foraminifera_model_2a_it1_filtered <- Filter(function(iteration) !is.na(iteration$result2[[1]]), foraminifera_model_2a_it1)
foraminifera_model_2a_it1_filtered <- Filter(function(iteration) !is.null(iteration), foraminifera_model_2a_it1_filtered)
foraminifera_model_2a_it1_results <- foraminifera_model_2a_it1_filtered[1:50]
...
for (j in 1:50) {
  AICc[j,1] <- j 
  AICc[j,2] <- foraminifera_model_2a_it1_results[[j]]$result2$AICc 
}
...
foraminifera_model_2a_best_it1 <- foraminifera_model_2a_it1_results[[AICc_min]]

#Saving the best iteration
print(foraminifera_model_2a_best_it1)
save(foraminifera_model_2a_best_it1, file = 'foraminifera_bestrun_model_2a_it1.RData')