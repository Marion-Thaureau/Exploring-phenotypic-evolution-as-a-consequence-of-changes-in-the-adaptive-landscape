########################################################
##                                                    ##
##  COCCOLITH SCRIPT - Model 4a (iteration 50 - 100)  ##
##                                                    ## 
########################################################
...
coccolith_model_4a_it50 <- foreach(i=1:104) %dopar% { 
...
save.image(file = 'coccolith_allruns_model_4a_it50.RData')



###### EXTRACTING AND SAVING THE ITERATION WITH BEST PARAMETER ESTIMATES ######
#Filtering 50 iterations which converged
coccolith_model_4a_it50_filtered <- Filter(function(iteration) !is.na(iteration$result2[[1]]), coccolith_model_4a_it50)
coccolith_model_4a_it50_filtered <- Filter(function(iteration) !is.null(iteration), coccolith_model_4a_it50_filtered)
coccolith_model_4a_it50_results <- coccolith_model_4a_it50_filtered[1:50]
...
for (j in 1:50) {
  AICc[j,1] <- j 
  AICc[j,2] <- coccolith_model_4a_it50_results[[j]]$result2$AICc 
}
...
coccolith_model_4a_best_it50 <- coccolith_model_4a_it50_results[[AICc_min]]

#Saving the best iteration
print(coccolith_model_4a_best_it50)
save(coccolith_model_4a_best_it50, file = 'coccolith_bestrun_model_4a_it50.RData')