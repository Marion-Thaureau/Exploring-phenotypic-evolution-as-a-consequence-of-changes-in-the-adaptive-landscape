########################################################
##                                                    ##
##  COCCOLITH SCRIPT - Model 5b (iteration 75 - 100)  ##
##                                                    ## 
########################################################
...
#Model_5b: carbon isotope influences the primary optimum of the body length in A matrix; full R matrix 
A = matrix(c(1,0,1,0,1,0,0,0,1), nrow = 3, byrow=TRUE)
R = matrix(c(1,1,1,1,1,1,1,1,1), nrow = 3, byrow=TRUE)

#Loop for 35 iterations 
coccolith_model_5b_it75 <- foreach(i=1:35) %dopar% { 
...
save.image(file = 'coccolith_allruns_model_5b_it75.RData')



###### EXTRACTING AND SAVING THE ITERATION WITH BEST PARAMETER ESTIMATES ######
#Filtering 25 iterations which converged
coccolith_model_5b_it75_filtered <- Filter(function(iteration) !is.na(iteration$result2[[1]]), coccolith_model_5b_it75)
coccolith_model_5b_it75_filtered <- Filter(function(iteration) !is.null(iteration), coccolith_model_5b_it75_filtered)
coccolith_model_5b_it75_results <- coccolith_model_5b_it75_filtered[1:25]

#Extracting the AICc values of each iterations
AICc <- matrix(ncol=2, nrow=25)
colnames(AICc) <- c("iteration", "AIC")

for (j in 1:25) {
  AICc[j,1] <- j 
  AICc[j,2] <- coccolith_model_5b_it75_results[[j]]$result2$AICc 
}
...
coccolith_model_5b_best_it75 <- coccolith_model_5b_it75_results[[AICc_min]]

#calculating the correlation coefficients 
Rmatrix_best_coccolith_model_5b_it75 <- coccolith_model_5b_best_it75$result2$R
cormatrix_best_coccolith_model_5b_it75 <- cov2cor(Rmatrix_best_coccolith_model_5b_it75)

#Saving the best iteration
print(coccolith_model_5b_best_it75)
print(cormatrix_best_coccolith_model_5b_it75)
save(coccolith_model_5b_best_it75, cormatrix_best_coccolith_model_5b_it75, file = 'coccolith_bestrun_model_5b_it75.RData')