########################################################
##                                                    ##
##  COCCOLITH SCRIPT - Model 6a (iteration 75 - 100)  ##
##                                                    ## 
########################################################
...
coccolith_model_6a_it75 <- foreach(i=1:35) %dopar% { 
...
#Run iterations using the parametrized matrices A and R
  result$result2 <- tryCatch({fit.multivariate.OU.user.defined(ln.bodylength_isotopes_multiobj, A.user = A, R.user = R, iterations = 1, hess = TRUE,
                                                               user.init.diag.A = c(0.66, 36.48, 22.23),
                                                               user.init.upper.diag.A = NULL,
                                                               user.init.diag.R = c(0.00, 6.46, 4.05),
...
save.image(file = 'coccolith_allruns_model_6a_it75_sp2.RData')



###### EXTRACTING AND SAVING THE ITERATION WITH BEST PARAMETER ESTIMATES ######
#Filtering 25 iterations which converged
coccolith_model_6a_it75_filtered <- Filter(function(iteration) !is.na(iteration$result2[[1]]), coccolith_model_6a_it75)
coccolith_model_6a_it75_filtered <- Filter(function(iteration) !is.null(iteration), coccolith_model_6a_it75_filtered)
coccolith_model_6a_it75_results <- coccolith_model_6a_it75_filtered[1:25]
...
for (j in 1:25) {
  AICc[j,1] <- j 
  AICc[j,2] <- coccolith_model_6a_it75_results[[j]]$result2$AICc 
}
...
coccolith_model_6a_best_it75 <- coccolith_model_6a_it75_results[[AICc_min]]

#Saving the best iteration
print(coccolith_model_6a_best_it75)
save(coccolith_model_6a_best_it75, file = 'coccolith_bestrun_model_6a_it75_sp2.RData')