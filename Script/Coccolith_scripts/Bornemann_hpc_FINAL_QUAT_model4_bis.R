...
OUOU.model1 <- foreach(i=1:100) %dopar% { #calculation for each iteration 
  ...
  result$result1 <- seed
  result$result2 <- fit.multivariate.OU.user.defined(multiENVI, A.user=A,R.user=R, iterations = 1, iter.sd = 1, hess = TRUE)
...
save.image(file='Bornemann_FINAL_QUAT_model4_bis.RData')
