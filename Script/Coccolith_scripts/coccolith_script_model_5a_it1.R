######################################################
##                                                  ##
##  COCCOLITH SCRIPT - Model 5a (iteration 1 - 50)  ##
##                                                  ## 
######################################################
...
coccolith_model_5a_it1 <- foreach(i=1:104) %dopar% { 
...
save.image(file = 'coccolith_allruns_model_5a_it1.RData')



###### EXTRACTING AND SAVING THE ITERATION WITH BEST PARAMETER ESTIMATES ######
#Filtering 50 iterations which converged
coccolith_model_5a_it1_filtered <- Filter(function(iteration) !is.na(iteration$result2[[1]]), coccolith_model_5a_it1)
coccolith_model_5a_it1_filtered <- Filter(function(iteration) !is.null(iteration), coccolith_model_5a_it1_filtered)
coccolith_model_5a_it1_results <- coccolith_model_5a_it1_filtered[1:50]
...
for (j in 1:50) {
  AICc[j,1] <- j 
  AICc[j,2] <- coccolith_model_5a_it1_results[[j]]$result2$AICc 
}
...
coccolith_model_5a_best_it1 <- coccolith_model_5a_it1_results[[AICc_min]]

#Saving the best iteration
print(coccolith_model_5a_best_it1)
save(coccolith_model_5a_best_it1, file = 'coccolith_bestrun_model_5a_it1.RData')