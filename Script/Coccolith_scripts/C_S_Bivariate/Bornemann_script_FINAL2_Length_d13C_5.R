...
multienvi=make.multivar.evoTS(ln.totallength, d13Ccarb)


## WITH ENVIRONMENTAL FACTOR AFFECTING THE OPTIMUM OF PHENOTYPIC TRAITS FOR NUMBER OF ENVIRONMENTAL VARIABLES REDUCED (2)
A = matrix(c(1,0,0,0), nrow = 2, byrow=TRUE) #1 for parametrised and 0 for not parametrised
R = matrix(c(1,0,0,1), nrow = 2, byrow=TRUE) #5 traits donc 25 cases dans la matrice
model5=fit.multivariate.OU.user.defined(multienvi,A.user=A,R.user=R, iterations=100)
...
save.image(file='Bornemann_FINAL_Length_d13C_5.RData')
