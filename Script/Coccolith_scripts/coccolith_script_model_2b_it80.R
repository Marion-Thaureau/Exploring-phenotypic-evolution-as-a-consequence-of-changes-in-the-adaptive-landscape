#######################################################
##                                                   ##
##  COCCOLITH SCRIPT - Model 2b (iteration 80 - 90)  ##
##                                                   ## 
#######################################################
...
coccolith_model_2b_it80 <- foreach(i=1:100) %dopar% { 
...
save.image(file = 'coccolith_allruns_model_2b_it80.RData')



###### EXTRACTING AND SAVING THE ITERATION WITH BEST PARAMETER ESTIMATES ######
#Filtering 10 iterations which converged
coccolith_model_2b_it80_filtered <- Filter(function(iteration) !is.na(iteration$result2[[1]]), coccolith_model_2b_it80)
coccolith_model_2b_it80_filtered <- Filter(function(iteration) !is.null(iteration), coccolith_model_2b_it80_filtered)
coccolith_model_2b_it80_results <- coccolith_model_2b_it80_filtered[1:10]
...
for (j in 1:10) {
  AICc[j,1] <- j 
  AICc[j,2] <- coccolith_model_2b_it80_results[[j]]$result2$AICc 
}
...
coccolith_model_2b_best_it80 <- coccolith_model_2b_it80_results[[AICc_min]]

#calculating the correlation coefficients 
Rmatrix_best_coccolith_model_2b_it80 <- coccolith_model_2b_best_it80$result2$R
cormatrix_best_coccolith_model_2b_it80 <- cov2cor(Rmatrix_best_coccolith_model_2b_it80)

#Saving the best iteration
print(coccolith_model_2b_best_it80)
print(cormatrix_best_coccolith_model_2b_it80)
save(coccolith_model_2b_best_it80, cormatrix_best_coccolith_model_2b_it80, file = 'coccolith_bestrun_model_2b_it80.RData')